...
//    @Override
    ...
    private void onLoginSuccess(String email, String password) {
        ...
        if (currentAccount == null) {
            currentAccount = new Account(email, AccountConstants.ACCOUNT_TYPE);
            am.addAccountExplicitly(currentAccount,
                    password,
                    null);
        } else {
            am.setPassword(currentAccount, password);
        }
    ...
    private String getAuthToken(String email, String password) {
        return email.substring(0, 2) + ":" + password.substring(0, Math.min(1, password.length()));
    }
